...
void WorldSession::HandleLfgLeaveOpcode(WorldPackets::LFG::DFLeave& dfLeave)
{
    ...
    // Check cheating/spoofing:
    // 1) the requester in the packet must be the current player
    // 2) only group leader can remove a grouped queue
    if (dfLeave.Ticket.RequesterGuid != playerGuid)
        return;

    if (group && group->GetLeaderGUID() != playerGuid)
        return;

    sLFGMgr->LeaveLfg(playerGuid, dfLeave.Ticket.Id);
}
